...
const Resource = ({
  ...
  const [resources, setResources] =
    useState<IResourcesReslut[]>(resourcesResult);
  const [resource, setResource] = useState<IResourceCreateRequest>({
    name: "",
    resource_key: "",
  });
  ...
  const submitResource = async () => {
    const response = await fetch(
      `${process.env.BASE_API}/${data?.user?.org_id}/resource`,
      {
        method: "POST",
        headers: {
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setResource({ name: resource.name, resource_key: e.target.value })
            }
            id="key"
            className="block w-full p-2 text-gray-50 border border-gray-500 rounded bg-gray-600 sm:text-xs focus:ring-yellow-500 focus:border-yellow-500"
            placeholder="Resource Name"
